...
void FlyCamera::Update(float deltaTime)
{
	...
	{
		worldSpeed = glm::vec3(glm::normalize(-GetWorldTransform()[2]) * (currentSpeed * deltaTime));
		SetPosition(glm::vec3(GetWorldTransform()[3] + glm::normalize(-GetWorldTransform()[2]) * (currentSpeed * deltaTime)));
	}
	else
	{
		worldSpeed = glm::vec3(0);
	}
	...
	{
		SetPosition(glm::vec3(GetWorldTransform()[3] + glm::normalize(-GetWorldTransform()[0]) * (currentSpeed * deltaTime)));
	}
	...
	/*Disabled for reason of being unable to access getMouseDelta
...
//-----------------------------------------------------------
// Sets current speed of camera
// return (glm:vec3): Return speed of camera
//-----------------------------------------------------------
glm::vec3 FlyCamera::GetCurrentSpeed()
{
	return worldSpeed;
}