...
//-----------------------------------------------------------
bool PhysicsScene::Plane2Plane(PhysicsObject* inObj1, PhysicsObject* inObj2)
{
	return false;
}
...
// return (bool): returns if collision occured
//-----------------------------------------------------------
bool PhysicsScene::Plane2Sphere(PhysicsObject* inObj1, PhysicsObject* inObj2)
{
	Sphere* sphere = dynamic_cast<Sphere*>(inObj2);
	Plane* plane = dynamic_cast<Plane*>(inObj1);
	...
	if (sphere != nullptr && plane != nullptr)
	{
		...
		if (intersection > 0)
		{
			...
			Rigidbody* rbSphere1 = dynamic_cast<Rigidbody*>(inObj2);
...
// return (bool): returns if collision occured
//-----------------------------------------------------------
bool PhysicsScene::Sphere2Plane(PhysicsObject* inObj1, PhysicsObject* inObj2)
{
	Sphere* sphere = dynamic_cast<Sphere*>(inObj1);
	Plane* plane = dynamic_cast<Plane*>(inObj2);
	//if we are successful then test for collision
	if (sphere != nullptr && plane != nullptr)
	{
		glm::vec2 collisionNormal = plane->GetNormal();
		...
		if (intersection > 0)
		{
			...
			Rigidbody* rbSphere1 = dynamic_cast<Rigidbody*>(inObj1);
			plane->ResolveCollision(rbSphere1, sphere->GetPosition() + (collisionNormal * -sphere->GetRadius()), nullptr);
			return true;
		}
	}
...
// return (bool): returns if collision occured
//-----------------------------------------------------------
bool PhysicsScene::Sphere2Sphere(PhysicsObject* inObj1, PhysicsObject* inObj2)
{
	Sphere* sphere1 = dynamic_cast<Sphere*>(inObj1);
	Sphere* sphere2 = dynamic_cast<Sphere*>(inObj2);
	//if we are successful then test for collision
	if (sphere1 != nullptr && sphere2 != nullptr)
	{
			...
			{
				Rigidbody* rbSphere1 = dynamic_cast<Rigidbody*>(inObj1);
				Rigidbody* rbSphere2 = dynamic_cast<Rigidbody*>(inObj2);
				...
				glm::vec2 norm = glm::normalize(contact - sphere1->GetPosition());
				rbSphere1->ResolveCollision(rbSphere2, contact);
				ApplyContactForces(rbSphere1, rbSphere2, norm, pen);
				return true;
			}
		}
	return false;
...
// return (bool): returns if collision occured
//-----------------------------------------------------------
bool PhysicsScene::Plane2Box(PhysicsObject* inObj1, PhysicsObject* inObj2)
{
	OOB* box = dynamic_cast<OOB*>(inObj2);
	Plane* plane = dynamic_cast<Plane*>(inObj1);
	//if we are successful then test for collision
...
// return (bool): returns if collision occured
//-----------------------------------------------------------
bool PhysicsScene::Sphere2Box(PhysicsObject* inObj1, PhysicsObject* inObj2)
{
	OOB* box = dynamic_cast<OOB*>(inObj2);
	Sphere* sphere = dynamic_cast<Sphere*>(inObj1);
	if (box != nullptr && sphere != nullptr) {
		...
		if (localPos.x < w2 && localPos.x > -w2)
		{
			...
			if (numContacts > 0)
			{
				...
				ApplyContactForces(dynamic_cast<Rigidbody*>(sphere), dynamic_cast<Rigidbody*>(box), norm, pen);
			}
			delete direction;
		}
	}
...
// return (bool): returns if collision occured
//-----------------------------------------------------------
bool PhysicsScene::Box2Plane(PhysicsObject* inObj1, PhysicsObject* inObj2)
{
	OOB* box = dynamic_cast<OOB*>(inObj1);
	Plane* plane = dynamic_cast<Plane*>(inObj2);
	//if we are successful then test for collision
	if (box != nullptr && plane != nullptr)
	{
...
// return (bool): returns if collision occured
//-----------------------------------------------------------
bool PhysicsScene::Box2Sphere(PhysicsObject* inObj1, PhysicsObject* inObj2)
{
	OOB* box = dynamic_cast<OOB*>(inObj1);
	Sphere* sphere = dynamic_cast<Sphere*>(inObj2);
	if (box != nullptr && sphere != nullptr) {
...
// return (bool): returns if collision occured.
//-----------------------------------------------------------
bool PhysicsScene::Box2Box(PhysicsObject* inObj1, PhysicsObject* inObj2) 
{
	OOB* box1 = dynamic_cast<OOB*>(inObj1);
	OOB* box2 = dynamic_cast<OOB*>(inObj2);
...
//-----------------------------------------------------------
// Get the actors in scene
// return (std::vector<PhysicsObject*>): Returns actors in scene
//-----------------------------------------------------------
