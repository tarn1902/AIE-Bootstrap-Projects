...
void Box::MakeGizmo()
{
	//glm::vec2(position.x - extents.x, position.y)
	aie::Gizmos::add2DAABBFilled(GetPosition(), GetExtents(), colour);
}
...
bool Box::CheckCollision(PhysicsObject* inPOther)
{
	...
	if (pBox != nullptr)
	{
		if (position.x - extents.x < pBox->GetPosition().x + pBox->GetExtents().x &&
			position.x + extents.x> pBox->GetPosition().x - pBox->GetExtents().x &&
			position.y - extents.y < pBox->GetPosition().y + pBox->GetExtents().y &&
			position.y + extents.y > pBox->GetPosition().y - pBox->GetExtents().y)
		{
