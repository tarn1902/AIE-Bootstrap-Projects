...
bool Box::CheckCollision(PhysicsObject* inPOther)
{
	...
	if (pBox != nullptr)
	{
		if (position.x < pBox->GetPosition().x + pBox->GetExtents().x * 2 &&
			position.x + extents.x * 2 > pBox->GetPosition().x &&
			position.y < pBox->GetPosition().y + pBox->GetExtents().y * 2 &&
			position.y + extents.y * 2 > pBox->GetPosition().y)
		{
