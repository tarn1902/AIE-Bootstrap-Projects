...
//-----------------------------------------------------------
void OOB::FixedUpdate(glm::vec2 gravity, float timeStep)
{
...
// return (glm::vec2): Returns the centerof object
//-----------------------------------------------------------
...
//-----------------------------------------------------------
bool OOB::CheckBoxCorners(OOB& box, glm::vec2& contact, int& numContacts, float& pen, glm::vec2& edgeNormal)
{
	...
	for (float x = -box.GetExtents().x; x < boxW; x += boxW)
	{
		for (float y = -box.GetExtents().y; y < boxH; y += boxH)
		{
			...
			glm::vec2 p = box.GetCenter() + x * box.localX + y * box.localY;
			// position in our box's space
			glm::vec2 p0(glm::dot(p - position, localX),
				glm::dot(p - position, localY));
	...
	if (numLocalContacts == 0)
		return false;
	bool res = false;
	contact += position + (localContact.x * localX + localContact.y * localY) /
